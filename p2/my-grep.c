...
void readFile(char *argv, char *search_term)
{
    ...
    { // Check if input stream is from stdin or a file
        f = stdin;
    ...
    { // Read lines till EOF
        ...
        { // Search for search term within line
            printf("%s\n", buffer);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    { // only search term specified --> read from stdin
        readFile("stdin", search_term);
    }
    else
    { // search term and n amount of filenames given
        while (argv[i] != NULL)
        {
